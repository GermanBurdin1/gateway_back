...
export class VocabularyGatewayController {
  private readonly logger = new Logger(VocabularyGatewayController.name);
  private readonly vocabularyServiceUrl = "http://localhost:3000";
  ...
  async proxyToVocabularyService(@Req() req: Request, @Res() res: Response) {
    ...
    try {
      ...
      this.logger.error(
        `[API Gateway] Ошибка при проксировании к vocabulary-service: ${error.message}`,
      );
